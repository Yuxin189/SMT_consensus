...
static Z3_ast mk_count_matches(Z3_context ctx, Z3_ast *recv_vec, int value) {
    Z3_ast terms[NUM_NODES];
    for (int k = 0; k < NUM_NODES; k++) {
        Z3_ast is_match = Z3_mk_eq(ctx, recv_vec[k], mk_int(ctx, value));
        terms[k] = Z3_mk_ite(ctx, is_match, mk_int(ctx, 1), mk_int(ctx, 0));
    }
    return Z3_mk_add(ctx, NUM_NODES, terms);
}

/* Canonical-pattern ITE: only depends on counts of 0 and 1, not sender order. */
static Z3_ast mk_pattern_ite(Z3_context ctx, const int *patterns, Z3_ast *recv_vec,
                             const Z3_ast *round_rules) {
    Z3_ast count0 = mk_count_matches(ctx, recv_vec, 0);
    Z3_ast count1 = mk_count_matches(ctx, recv_vec, 1);
    Z3_ast nested = round_rules[g_num_patterns - 1];
    for (int p_idx = g_num_patterns - 2; p_idx >= 0; p_idx--) {
        int p_off = p_idx * 2;
        Z3_ast match0 = Z3_mk_eq(ctx, count0, mk_int(ctx, patterns[p_off]));
        Z3_ast match1 = Z3_mk_eq(ctx, count1, mk_int(ctx, patterns[p_off + 1]));
        Z3_ast match = Z3_mk_and(ctx, 2, (Z3_ast[]){match0, match1});
