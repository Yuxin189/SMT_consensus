...
int main(void) {
    ...
    double tot_synth_constraints_count = 0, tot_verify_constraints_count = 0;
    ...
    for (;;) {
        ...
        if (!synthesize(ctx, counter_examples, num_cex, patterns, &candidate, &t_synth)) {
            ...
            printf("  Final synth ast nodes: %llu\n", final_synth_ast_nodes);
            printf("  Final verify ast nodes: %llu\n", final_verify_ast_nodes);
            printf("  Constraint count time: synth=%.6fs verify=%.6fs\n", tot_synth_constraints_count, tot_verify_constraints_count);
            printf("  AST count time: synth=%.6fs verify=%.6fs\n", tot_synth_ast_count, tot_verify_ast_count);
        ...
        tot_synth_constraints_count += t_synth.constraints_count;
        tot_synth_ast_count += t_synth.ast_count;
        final_synth_constraints = t_synth.constraints;
        final_synth_ast_nodes = t_synth.ast_nodes;
        ...
        tot_verify_constraints_count += t_verify.constraints_count;
        tot_verify_ast_count += t_verify.ast_count;
        final_verify_constraints = t_verify.constraints;
        final_verify_ast_nodes = t_verify.ast_nodes;
